...
describe('MeetAdapter Test', function () {
    // Constant representing a mock Endpoint ID for testing purposes
    const eidA = 1
    const eidB = 2
    // Declaration of variables to be used in the test suite
    let MeetAdapter: ContractFactory
    let Meet: ContractFactory
    ...
    beforeEach(async function () {
        ...
        // Deploying two instances of Meet contract with different identifiers and linking them to the mock LZEndpoint
        myOFTAdapter = await MeetAdapter.deploy(token.address, mockEndpointV2A.address, ownerA.address)
        myOFTB = await Meet.deploy('bOFT', 'bOFT', mockEndpointV2B.address, ownerB.address)

        // Setting destination endpoints in the LZEndpoint mock for each Meet instance
        ...
        // Setting each Meet instance as a peer of the other in the mock LZEndpoint
